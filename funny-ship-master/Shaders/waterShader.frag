...
void main() {
    ...
    if(useTextures == 0) {
        color = vec4((ambient + diffuse).xyz * lightIntensity, 1);
    } else {
        color = vec4(((ambient + diffuse * texture(colorTextureUnit, intUv)).xyz + (specular * texture(specularTextureUnit, intUv)).xyz) * lightIntensity, 1);
    }
}
